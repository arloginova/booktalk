...
const authorCls = 'lg-low:mt-3 mt-1 lg-low:mb-5 mb-3 text-greyExtra lg-low:text-xl text-sm'
const ratingCls = 'flex items-center justify-between'
const generalRatingCls = 'lg-low:text-2xl text-sm flex items-center lg-low:gap-x-1 gap-x-0.5 leading-none'
const starsCls = 'flex items-center lg-low:gap-x-1 gap-x-0.5 text-xl text-greyExtra'
const descriptionCls = 'lg-low:my-5 my-3 lg-low:text-xl text-sm'
const expandCls = 'lg-low:text-xl text-sm'

const Info = ({ className, description, slug, author, userReviews }) => {
	return (
		<div className={cn(wrapperCls, className)}>
			...
			<div className={ratingCls}>
				<div className={generalRatingCls}>
					<StarIcon fill='var(--color-blackMain)' className='lg-low:size-6 size-5' />
				...
				<div className={starsCls}>
					{[...Array(5)].map((_, index) => (
						<StarIcon fill='var(--color-greyBackground)' key={index} className='lg-low:size-6 size-5' />
					))}
					<UiTypography font='Raleway-M' tag='p' className='lg-low:text-base text-sm lg-low:ml-2 ml-0.5'>
						Оценить
					</UiTypography>
				</div>
			</div>
